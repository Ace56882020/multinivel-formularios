...
export class AppComponent implements OnInit {
  ...
  addZ(ix, iy) {
    const control = (
      (<FormArray>this.form.controls['Xs']).at(ix).get('Ys') as FormArray
    )
      .at(iy)
      .get('Zs') as FormArray;
  ...
  validateXs() {
    ...
    while (x <= XsA.length) {
      ...
      for (let field in X.controls) {
        ...
        if (input.invalid && input.dirty) {
          for (let error in input.errors) {
            this.formErrors.Xs[x - 1][field] =
              this.validationMessages.Xs[field][error];
          }
        }
      }
      this.validateYs(x);
  ...
  validateYs(x) {
    ...
    while (y <= YsA.length) {
      ...
      for (let field in Y.controls) {
        ...
        if (input.invalid && input.dirty) {
          for (let error in input.errors) {
            this.formErrors.Xs[x - 1].Ys[y - 1][field] =
  ...
  validateZs(x, y) {
    ...
    let ZsA = (
      (<FormArray>this.form.controls['Xs']).at(x - 1).get('Ys') as FormArray
    )
      .at(y - 1)
      .get('Zs') as FormArray;
    ...
    while (z <= ZsA.length) {
      ...
      for (let field in Z.controls) {
        ...
        if (input.invalid && input.dirty) {
          for (let error in input.errors) {
            this.formErrors.Xs[x - 1].Ys[y - 1].Zs[z - 1][field] =
